...
class BooksController extends AbstractFOSRestController
{
     ...
     *@Rest\View(serializerGroups={"book"}, serializerEnableMaxDepthChecks=true)
     *  */
    public function getSingleAction(string $id, BookFormProcessor $bfp, BookManager $bookManager)
    {
     ...
     *@Rest\View(serializerGroups={"book"}, serializerEnableMaxDepthChecks=true)
     *  */
    public function editAction(string $id, BookFormProcessor $bfp, BookManager $bookManager, Request $request)
    {
        $book = $bookManager->find(Uuid::fromString($id));
        if (!$book) {
     ...
     *@Rest\View(serializerGroups={"book"}, serializerEnableMaxDepthChecks=true)
     *  */
    public function deleteAction(string $id, BookManager $bookManager, Request $request)
    {
        $book = $bookManager->find(Uuid::fromString($id));
        if (!$book) {
            return View::create('Book not found', Response::HTTP_BAD_REQUEST);
